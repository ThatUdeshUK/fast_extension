...
public class QueriesReader extends FileSpout {
	...
	//
	//		ImportableQuery q = buildQuery(line);
	//		for (int j = 0; j < 4 && i < this.totalQueryCountVal; j++) {
	//			ImportableQuery qold = q;
	//			q = new ImportableQuery(qold);
	//			q.setId(i+selfTaskIndex*totalQueryCountVal);
	//			this.collector.emit(new Values(q.getId(), q
	//			), i);
	//			i = i + 1;
	//			
	//		}
	...
	private void emitQuery(String line, Integer msgId) {
		...
		//	q = new ImportableQuery(qold);
		if (q == null) {
			System.out.println("null ImportableQuery");
			return;

		}
		q.setQueryId(i + selfTaskIndex * totalQueryCountVal);
		i = i + 1;
		//		this.collector.emit(new Values(q.getQueryType(), q.getId(), q.getFocalPoint().x, q.getFocalPoint().y, q.getSpatialRange().getMin().x, q.getSpatialRange().getMin().y, q.getSpatialRange().getMax().x,
		//				q.getSpatialRange().getMax().y, q.getK(), TextHelpers.convertArrayListOfStringToText(q.getKeywords()), TextHelpers.convertArrayListOfStringToText(q.getQueryText2()), q.getSt(), q.getDataSrc(), q.getDataSrc2(),
		//				q.getCommand(), q.getDistance(), q.getPredicate(), q.getTextualPredicate2(), q.getJoinTextualPredicate(),q.getRemoveTime()
		//
		//		));
	...
	public ImportableQuery buildQuery(String line, String scId, int keywordCount, String data1, String data2, Double distance, QueryType queryType, Double spatialRangeVal, TextualPredicate textualPredicate1, TextualPredicate textualPredicate2,
									  Integer k) {
	...
	public ImportableQuery buildQueryFromSynthetic(String line, String scId, int keywordCount, String data1, String data2, Double distance, QueryType queryType, Double spatialRangeVal, TextualPredicate textualPredicate1, TextualPredicate textualPredicate2,
												   Integer k) {
		if (previousLocations == null || selfTaskId == null || selfTaskIndex == null) {
			previousLocations = new ArrayList<LatLong>();
			r = new RandomGenerator(0);
		...
		QueriesReader.textualPredicate1 = textualPredicate1;
		//QueriesFileSystemSpout.textualPredicate2 = textualPredicate2;
		ImportableQuery q = buildQueryfromSynthetic(line);
		if (q != null) {
			q.setQueryId(i + selfTaskIndex * totalQueryCountVal);
			i = i + 1;
			q.setSrcId(scId);
	...
	public ImportableQuery buildQueryFromPoi(String line, String scId, int keywordCount, String data1, String data2, Double distance, QueryType queryType, Double spatialRangeVal, TextualPredicate textualPredicate1, TextualPredicate textualPredicate2,
											 Integer k) {
		if (previousLocations == null || selfTaskId == null || selfTaskIndex == null) {
			previousLocations = new ArrayList<LatLong>();
			r = new RandomGenerator(0);
		...
		QueriesReader.textualPredicate1 = textualPredicate1;
		//QueriesFileSystemSpout.textualPredicate2 = textualPredicate2;
		ImportableQuery q = buildQueryFromPoi(line);
		if (q != null) {
			q.setQueryId(i + selfTaskIndex * totalQueryCountVal);
			i = i + 1;
			q.setSrcId(scId);
	...
	public ImportableQuery buildQuery(String line) {
		//String[] tweetParts = line.split(",");
				try {
					...
					if (queryType.equals(QueryType.queryTextualRange)) {
						q.setSpatialRange(new Rectangle(xy, new Point(xy.x + spatialRangeVal, xy.y + spatialRangeVal)));
	...
	public ImportableQuery buildQueryfromSynthetic(String line) {
		//String[] tweetParts = line.split(",");
		try {
	...
	public ImportableQuery buildQueryFromPoi(String line) {
		//String[] tweetParts = line.split(",");
		try {
	...
	//	private ImportableQuery buildQuery(String line) {
	//		StringTokenizer stringTokenizer = new StringTokenizer(line, ",");
	//
	...
	//			q.setPredicate(textualPredicate1);
	//			q.setKeywords(queryText1);
