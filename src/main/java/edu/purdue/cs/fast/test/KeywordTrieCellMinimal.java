...
public class KeywordTrieCellMinimal {
	...
	public void find(List<String> keywords, int start, LinkedList<MinimalRangeQuery> result, int level) {
		if (storedQueries != null) {
			result.addAll(storedQueries);
		}
		if (verifiableQueries != null)
			for (MinimalRangeQuery q : verifiableQueries) {
				if (TextHelpers.containsTextually(keywords, q.keywords))
					result.add(q);
			}
		int i = start;
		if (trieCells != null)
	...
	public void find(List<String> keywords, int start, LinkedList<MinimalRangeQuery> result, int level, Integer[]opCount) {
		...
		if (verifiableQueries != null) {
			...
			for (MinimalRangeQuery q : verifiableQueries) {
				...
				//opCount[0]+=(q.keywords.size()+keywords.size());
				if (TextHelpers.containsTextually(keywords, q.keywords))
					result.add(q);
			}
		}
