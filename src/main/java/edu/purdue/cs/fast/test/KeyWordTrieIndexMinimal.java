...
public class KeyWordTrieIndexMinimal {
	...
	public static int SPLIT_THRESHOLD_FREQ =0;
	int size = 0;
	...
	public void insert(List<String> keywordList, MinimalRangeQuery q) {
		...
		for (String keyword : keywordList) {
			...
			if (currentCell.trieCells == null && i<keywordList.size()-1&&(currentCell.verifiableQueries == null || currentCell.verifiableQueries.size() <= SPLIT_THRESHOLD)) {
				...
				for (MinimalRangeQuery otherQuery : currentCell.verifiableQueries) {
					KeywordTrieCellMinimal otherCell = currentCell.trieCells.get(otherQuery.keywords.get(i + 1));
					if (otherCell == null) {
						otherCell = new KeywordTrieCellMinimal();
						currentCell.trieCells.put(otherQuery.keywords.get(i + 1), otherCell);
					}
					if (i + 1 == otherQuery.keywords.size() - 1) {//last keyword
	...
	public void insertNonExpandable(ArrayList<String> keywordList, MinimalRangeQuery q) {
		KeywordTrieCellMinimal currentCell = root;
		size++;
		...
		if (currentCell.trieCells == null) {
			...
			if (currentCell.storedQueries.size() > SPLIT_THRESHOLD) {
				...
				for (MinimalRangeQuery query : currentCell.storedQueries) {
					if (query.keywords.size() > level) {
						if (!currentCell.trieCells.containsKey(query.keywords.get(level))) {

							currentCell.trieCells.put(query.keywords.get(level), new KeywordTrieCellMinimal());
						}
						currentCell.trieCells.get(query.keywords.get(level)).storedQueries.add(query);
					} else {
	...
	public LinkedList<MinimalRangeQuery> find(List<String> keywordList) {
		LinkedList<MinimalRangeQuery> result = new LinkedList<MinimalRangeQuery>();
		
		root.find(keywordList, 0, result, 0);
		return result;
	}
	public LinkedList<MinimalRangeQuery> find(List<String> keywordList,Integer[] opCount) {
		LinkedList<MinimalRangeQuery> result = new LinkedList<MinimalRangeQuery>();
		root.find(keywordList, 0, result, 0,opCount);
		return result;
	}
